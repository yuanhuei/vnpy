...
class MyBollingMultiPosStrategy(CtaTemplate):
    """基于布林通道的交易策略"""
    className = 'MyBollingMultiPosStrategy'
    author = 'yuanhui'
    ...
    def on_30Min_bar(self, bar: BarData):
        ...
        if self.pos==0:  #无仓位
            ...
            if trade.offset==Offset.OPEN:
                if ((self.beforebolldown30 <=trade.price and self.bollDown30 >trade.price) and trade.direction==Direction.LONG) or ((self.beforebollup30 >=trade.price and self.bollUp30 <trade.price) and trade.direction==Direction.SHORT):
            ...
            elif trade.offset==Offset.OPEN and (trade.direction==Direction.LONG and self.lastTrade_baoben==False): 
                orderList=self.sell(self.longExit, trade.volume, True)
                print (u"策略：%s,委托止损单，30分钟下轨平仓"%self.className)
            # 最后一笔交易为空头仓位，没有保本，在上轨止损
            elif trade.offset==Offset.OPEN and (trade.direction==Direction.SHORT and self.lastTrade_baoben==False): 
            ...
            if len(self.tradedata_baoben)>0:
                ...
                while i <len(self.tradedata_day):
                    ...
                    if self.tradedata_baoben[i].direction==Direction.LONG:
                        orderList=self.sell(max(self.tradedata_baoben[i].price,bollMidDay), volume, True)
                        print (u"策略：%s,委托止损单，保本价格平仓"%self.className)          
                    elif self.tradedata_baoben[i].direction==Direction.SHORT:
                        orderList=self.cover(min(self.tradedata_baoben[i].price,bollMidDay), volume, True)
                        print (u"策略：%s,委托止损单，保本价格平仓"%self.className)
                    i=i+1    
