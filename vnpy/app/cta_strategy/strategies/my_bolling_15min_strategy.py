...
class MyBolling15MinStrategy(CtaTemplate):
    """基于布林通道的交易策略"""
...
#----------------------------------------------------------------------
    ...
    def on_15Min_bar(self, bar: BarData):
        ...
        if self.pos==0:  #无仓位
            ...
            if trade.offset==Offset.OPEN:
                if ( self.bollDown15 >trade.price and trade.direction==Direction.LONG) or (self.bollUp15 <trade.price and trade.direction==Direction.SHORT):
        ...
        current_path =os.getcwd()# os.path.abspath(__file__)
        bardata=[bar.datetime,self.FifteenMinTrendStatus,bar.open_price, bar.close_price, bar.high_price, bar.low_price,self.pos,int(self.bollDown15),int(self.bollMid15),int(self.bollUp15),self.dealopen]
    ...
    def onDayBar(self, bar: BarData):
        """日K线推送"""
        t1=str(bar.datetime)
        t2=str(datetime(2011,1,7,9,00,0))
        if t2 in t1:
            i=0        
        ...
        zhishun=self.bollUpDay-self.bollDownDay
        volume=self.caculate_pos(zhishun)            
        #日线盘整，上下轨开仓  
    ...
    def on_trade(self, trade):
        ...
        if trade.offset==Offset.OPEN and trade.direction==Direction.LONG: #多头成交，设置止损单
            orderList=self.sell(self.bollDown15-self.priceTick, trade.volume, True)
            print (u"委托止损单，15分钟下轨平仓")
            if orderList:
                print( u"委托单成功单号",orderList)    
            else :
                print (u"委托单失败")                  
        elif trade.offset==Offset.OPEN and trade.direction==Direction.SHORT: #空头成交，设置止损单
            orderList=self.cover(self.bollUp15+self.priceTick, trade.volume, True)
            print (u"委托止损单，15分钟上轨平仓")
            if orderList:
                print( u"委托单成功单号",orderList)    
            else :
                print (u"委托单失败")             
